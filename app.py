...
from auth import login_required, logout, init_auth_session, show_admin_panel
...
def main_app():
    ...
    with st.sidebar:
        ...
        if st.session_state.user_info:
            st.markdown("---")
            st.markdown(f"👤 **Usuario:** {st.session_state.user_info.get('email', 'Usuario')}")
            st.markdown(f"📅 **Último login:** {st.session_state.user_info.get('last_login', 'N/A')[:10]}")
